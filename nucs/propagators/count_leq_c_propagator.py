...
def _compute_domains_count_leq_c_plain(domains: NDArray, parameters: NDArray, prop_state: NDArray) -> int:
    """
...
def compute_domains_count_leq_c(domains: NDArray, parameters: NDArray, prop_state: NDArray) -> int:
    ...
    if len(prop_state) < STATE_LIVE + len(domains):
        return _compute_domains_count_leq_c_plain(domains, parameters, prop_state)
    a = int(parameters[0])
    c = int(parameters[1])
    n = len(domains)
    ...
    # count_min only rises down a branch and count_max only falls, so a carried count_min above c proves
    # the true one is too, and a carried count_max at or below c proves the same of the true one
    if count_min > c:
        return PROP_INCONSISTENCY
    if count_max <= c:
        return PROP_ENTAILMENT
