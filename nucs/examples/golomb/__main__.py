...
if __name__ == "__main__":
    ...
    solver = (
        MultiprocessingSolver(
            [
                BacktrackSolver(prob, consistency_alg_idx=consistency_alg_golomb, log_level=args.log_level)
                for prob in problem.split(args.processors, 0)
            ]
        )
        if args.processors > 1
        else BacktrackSolver(problem, consistency_alg_idx=consistency_alg_golomb, log_level=args.log_level)
    )
