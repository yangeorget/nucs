...
class TestEqCReif(PropagatorTest):
    @pytest.mark.parametrize(
        ...
        [
            ...
            # b=0, x[DOMAIN_MIN]==c -> x[DOMAIN_MIN] tightened, then x>c so b=0 entailed
            ([(0, 0), (4, 7)], [4], PROP_ENTAILMENT, [[0, 0], [5, 7]]),
            # b=0, x[DOMAIN_MAX]==c -> x[DOMAIN_MAX] tightened, then x<c so b=0 entailed
            ([(0, 0), (2, 4)], [4], PROP_ENTAILMENT, [[0, 0], [2, 3]]),
            # b=0, x fixed at c -> inconsistency after x[DOMAIN_MIN] += 1
