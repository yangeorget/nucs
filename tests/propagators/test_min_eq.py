...
from nucs.memory import PROP_CONSISTENCY, PROP_INCONSISTENCY, new_data_by_values, new_domains_by_values
...
class TestMinEQ:
    def test_compute_domains_1(self) -> None:
        ...
        assert compute_domains_min_eq(domains, data) == PROP_CONSISTENCY
        assert np.all(domains == np.array([[2, 4], [2, 5], [2, 4]]))

    def test_compute_domains_2(self) -> None:
        domains = new_domains_by_values([(1, 3), (3, 3), (4, 5)])
        data = new_data_by_values([])
        assert compute_domains_min_eq(domains, data) == PROP_INCONSISTENCY
