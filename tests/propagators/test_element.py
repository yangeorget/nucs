...
class TestElement:
    def test_compute_domains_1(self) -> None:
        ...
        assert compute_domains_element(domains, data) == PROP_CONSISTENCY
        assert np.all(domains == np.array([[-1, 0], [1, 2], [0, 1], [-1, 1]]))

    def test_compute_domains_2(self) -> None:
        domains = new_domains_by_values([(-4, -2), (1, 2), (0, 1), (0, 1)])
        data = new_data_by_values([])
        assert compute_domains_element(domains, data) == PROP_ENTAILMENT
    ...
    def test_compute_domains_3(self) -> None:
        ...
        data = new_data_by_values([])
        assert compute_domains_element(domains, data) == PROP_INCONSISTENCY
